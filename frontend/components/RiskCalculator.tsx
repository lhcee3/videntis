...
export default function RiskCalculator() {
  ...
  const riskLabel = useMemo(() => {
    ...
    if (r <= 2) return { text: "Standard", color: "#9ca3af" }
  ...
  return (
    <div className="border border-[#1e1e1e] p-6">
      <span className="font-mono text-[10px] text-[#9ca3af] uppercase tracking-[0.2em] block mb-6">(05) RISK CALCULATOR</span>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">

        {/* Position Sizer */}
        <div>
          <div className="font-mono text-[10px] text-[#9ca3af] mb-4">POSITION SIZER</div>
          <div className="space-y-3">
            <div>
              <label className="font-mono text-[10px] text-[#9ca3af] block mb-1">PORTFOLIO SIZE ($)</label>
              <input value={portfolio} onChange={e => setPortfolio(e.target.value)} className={inputClass} type="number" placeholder="10000" />
            </div>
            <div>
              <div className="flex items-center mb-1">
                <label className="font-mono text-[10px] text-[#9ca3af]">RISK PER TRADE (%)</label>
            ...
            <div>
              <label className="font-mono text-[10px] text-[#9ca3af] block mb-1">ENTRY PRICE ($)</label>
              <input value={entry} onChange={e => setEntry(e.target.value)} className={inputClass} type="number" placeholder="100" />
            </div>
            <div>
              <label className="font-mono text-[10px] text-[#9ca3af] block mb-1">STOP LOSS ($)</label>
          ...
          {pos.shares > 0 && (
            <div className="mt-5 border border-[#1e1e1e] border-l-2 border-l-[#00ff87] p-4 space-y-2">
              <div className="flex justify-between">
                <span className="font-mono text-[10px] text-[#9ca3af]">SHARES TO BUY</span>
                <span className="font-display text-2xl text-[#00ff87]">{pos.shares}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-mono text-[10px] text-[#9ca3af]">POSITION SIZE</span>
                <span className="font-mono text-sm">${pos.positionSize.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-mono text-[10px] text-[#9ca3af]">DOLLAR AT RISK</span>
                <span className="font-mono text-sm text-[#ff4444]">${pos.riskAmt.toFixed(0)}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-mono text-[10px] text-[#9ca3af]">MAX LOSS</span>
        ...
        {/* R/R Planner */}
        <div>
          <div className="font-mono text-[10px] text-[#9ca3af] mb-4">RISK / REWARD PLANNER</div>
          <div className="space-y-3">
            <div>
              <label className="font-mono text-[10px] text-[#9ca3af] block mb-1">ENTRY ($)</label>
              <input value={entry} onChange={e => setEntry(e.target.value)} className={inputClass} type="number" placeholder="100" />
            </div>
            <div>
              <label className="font-mono text-[10px] text-[#9ca3af] block mb-1">STOP LOSS ($)</label>
            ...
            <div>
              <label className="font-mono text-[10px] text-[#9ca3af] block mb-1">TARGET ($)</label>
          ...
          {rr && (
            <div className="mt-5 border border-[#1e1e1e] p-4 space-y-4">
              <div className="flex items-end justify-between">
                <div>
                  <div className="font-mono text-[10px] text-[#9ca3af] mb-1">RISK : REWARD</div>
                ...
                <div className="text-right">
                  ...
                  <div className="font-mono text-[10px] text-[#9ca3af] mt-1">Min win rate: {rr.minWinRate}%</div>
                </div>
              </div>

              <div>
                <div className="font-mono text-[10px] text-[#9ca3af] mb-1">RISK vs REWARD</div>
