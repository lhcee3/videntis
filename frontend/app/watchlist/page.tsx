    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
      <div className="text-center">
        ...
        <button onClick={() => signInWithGoogle()}
          className="font-mono text-sm border border-[#1e1e1e] px-6 py-3 hover:border-[#333] transition-colors text-[#f0ede8]">
          Sign In with Google
        </button>
      </div>
    </div>
      ...
      <main className="max-w-5xl mx-auto px-8 py-8">
        ...
        {tickers.length === 0 ? (
          <div className="border border-[#1e1e1e] p-12 text-center">
            ...
            <Link href="/" className="font-mono text-xs border border-[#1e1e1e] px-6 py-3 hover:border-[#333] transition-colors">
              Search stocks →
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tickers.map(ticker => (
              <div key={ticker} className="border border-[#1e1e1e] p-6 hover:border-[#333] transition-colors group">
